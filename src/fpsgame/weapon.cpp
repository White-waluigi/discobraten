...
namespace game
{
	...
	VARP(maxdebris, 10, 25, 100000);
	VARP(maxbarreldebris, 5, 10, 100000);

	ICOMMAND(getweapon, "", (), intret(player1->gunselect));

	vec rainbowGradient(float input) {
		float red, green, blue;

		// Calculate RGB values based on the input float
		if (input <= 0.2) {
			red = 1.0;
			green = input / 0.2;
			blue = 0.0;
	...
	int convertToRGB(vec v) {
		int red = (int)(v.x * 255);
		int green = (int)(v.y * 255);
		int blue = (int)(v.z * 255);

		// Create the RGB integer using bit manipulation
	...
	void updatebouncers(int time)
	{
		loopv(bouncers)
		{
			...
			{
				//regular_particle_splash(PART_STREAK,1 , 150000, pos, convertToRGB(bnc.color), 1, 1, 1);

			}
			regular_particle_splash(PART_FLAME, 2, 150, pos, convertToRGB(bnc.color), 3.4f, 50, -20);
			//regular_particle_splash(PART_SMOKE, 1, 150, pos, convertToRGB(bnc.color), 2.4f, 50, -20);
			//regularshape(PART_STREAK, 20, convertToRGB(bnc.color), 21, 100, 5, pos, 3, 1);
			...
			if(stopped)
			{
				if(bnc.bouncetype==BNC_GRENADE)
				{
					...
					hits.setsize(0);
					explode(bnc.local, bnc.owner, bnc.o, NULL, qdam, GUN_GL,bnc.color);
	...
	void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun,vec color)
	{
		particle_splash(PART_SPARK, 200, 300, v, 0xB49B4B, 0.24f);
		playsound(gun!=GUN_GL ? S_RLHIT : S_FEXPLODE, &v);

		particle_fireball(v, guns[gun].exprad, gun!=GUN_GL ? PART_EXPLOSION : PART_EXPLOSION_BLUE, gun!=GUN_GL ? -1 : int((guns[gun].exprad-4.0f)*15), gun!=GUN_GL ? convertToRGB(color): convertToRGB(color), 4.0f);
	...
	{
		explode(local, owner, v, safe, damage, gun, vec(0,0,0));
	}
	...
	void projsplash(projectile &p, vec &v, dynent *safe, int damage)
	{
		...
		{
			explode(p.local, p.owner, v, safe, damage, GUN_RL,p.color);
			...
			case GUN_RL:
				loopv(projs)
				{
					...
					if(p.gun == gun && p.owner == d && p.id == id && !p.local)
					{
						...
						explode(p.local, p.owner, pos, NULL, 0, GUN_RL,p.color);
	...
	void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id, int prevaction)     // create visual effect from a shot
	{
		...
		switch(gun)
		{
			case GUN_FIST:
				...
				loopv(bouncers)
				{
					if(d==bouncers[i]->owner) {
						bouncer &bnc = *bouncers[i];
						bnc.lifetime=rnd(3000);
					}
				}
				break;
			...
			case GUN_SG:
				{
					...
					if(muzzleflash && d->muzzle.x >= 0)
						particle_flare(d->muzzle, d->muzzle, 200, PART_MUZZLE_FLASH3, /*0xFFFFFF*/ i_color, 2.75f, d);
					loopi(guns[gun].rays)
					{
						particle_splash(PART_SPARK, 20, 250, rays[i], /*0xB49B4B*/ i_color, 0.24f);
						particle_flare(hudgunorigin(gun, from, rays[i], d), rays[i], 300, PART_STREAK, /*0xFFC864*/ i_color, 0.28f);
						if(!local) adddecal(DECAL_BULLET, rays[i], vec(from).sub(rays[i]).safenormalize(), 2.0f);
					}
					if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 30, /*vec(0.5f, 0.375f, 0.25f)*/ v_color, 100, 100, DL_FLASH, 0, vec(0, 0, 0), d);
					break;
				}
			...
			case GUN_PISTOL:
				{
					...
					particle_splash(PART_SPARK, 200, 250, to, /*0xB49B4B*/i_color, 0.24f);
					particle_flare(hudgunorigin(gun, from, to, d), to, 600, PART_STREAK, /*0xFFC864*/ i_color, 0.28f);
					if(muzzleflash && d->muzzle.x >= 0)
						particle_flare(d->muzzle, d->muzzle, gun==GUN_CG ? 100 : 200, PART_MUZZLE_FLASH1, /*0xFFFFFF*/ i_color, gun==GUN_CG ? 2.25f : 1.25f, d);
					if(!local) adddecal(DECAL_BULLET, to, vec(from).sub(to).safenormalize(), 2.0f);
					if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), gun==GUN_CG ? 30 : 15, /*vec(0.5f, 0.375f, 0.25f)*/v_color, gun==GUN_CG ? 50 : 100, gun==GUN_CG ? 50 : 100, DL_FLASH, 0, vec(0, 0, 0), d);
					break;
				}

			case GUN_RL:
				if(muzzleflash && d->muzzle.x >= 0)
					particle_flare(d->muzzle, d->muzzle, 250, PART_MUZZLE_FLASH2, /*0xFFFFFF*/ i_color, 3.0f, d);
			case GUN_FIREBALL:
			case GUN_ICEBALL:
			case GUN_SLIMEBALL:
			...
			case GUN_GL:
				{
					...
					up.z += dist/8;
					if(muzzleflash && d->muzzle.x >= 0)
						particle_flare(d->muzzle, d->muzzle, 200, PART_MUZZLE_FLASH2, /*0xFFFFFF*/ i_color, 1.5f, d);
					if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 20, /*vec(0.5f, 0.375f, 0.25f)*/v_color, 100, 100, DL_FLASH, 0, vec(0, 0, 0), d);
			...
			case GUN_RIFLE:
				...
				particle_trail(PART_SMOKE, 500, hudgunorigin(gun, from, to, d), to, /*0x404040*/ i_color, 0.6f, 20);
				if(muzzleflash && d->muzzle.x >= 0)
					particle_flare(d->muzzle, d->muzzle, 150, PART_MUZZLE_FLASH3, /*0xFFFFFF*/ i_color, 1.25f, d);
				if(!local) adddecal(DECAL_BULLET, to, vec(from).sub(to).safenormalize(), 3.0f);
				if(muzzlelight) adddynlight(hudgunorigin(gun, d->o, to, d), 25, /*vec(0.5f, 0.375f, 0.25f)*/ v_color, 75, 75, DL_FLASH, 0, vec(0, 0, 0), d);
				break;
		}
	...
	void renderbouncers()
	{
		...
		loopv(bouncers)
		{
			...
			if(bnc.bouncetype==BNC_GRENADE)
			{
				particle_splash(PART_SPARK, 200, 300, bnc.offset, 0xB49B4B, 0.24f);
